...
int main(int argc, char **argv) { // 넘어오는 인자의 개수와 인자 배열 포인팅
  int listenfd, connfd;
  ...
  if (argc != 2) { // 인자 개수가 2개 인지 체크(포트 번호가 입력됐어?)
    fprintf(stderr, "usage: %s <port>\n", argv[0]);
    exit(1);
  }

  listenfd = Open_listenfd(argv[1]); // open_listenfd 함수를 호출해서 듣기 소켓을 오픈한다.
  while (1) { // Tiny는 전형적인 무한 서버 루프를 실행하고,
...
void doit(int fd)
{
  ...
  if (is_static) { // Serve static content
    if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) { // 우리는 이 파일이 보통 파일이라는 것과 읽기 권한을 가지고 있는지를 검증한다. 
      clienterror(fd, filename, "403", "Forbidden", "Tiny couldn't read the file");
      return;
    }
    serve_static(fd, filename, sbuf.st_size); // 만일 그렇다면(파일이 보통 파일이라는 것과 읽기 권한을 가지고 있다면), 정적 컨텐츠를 클라이언트에게 제공한다.
  }
  else {// 만일 요청이 동적 컨텐츠에 대한 것이라면
    if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) { // 이 파일이 실행 가능한지 검증하고, 
      clienterror(fd, filename, "403", "Forbidden", "Tiny couldn't run the CGI program");
      return;
    }
    serve_dynamic(fd, filename, cgiargs); // 만일 실행가능하다면 진행해서 동적 컨텐츠를 제공한다.
  }
}
...
int parse_uri(char *uri, char *filename, char *cgiargs) {
  ...
  if (!strstr(uri, "cgi-bin")) { // 만일 요청이 정적 컨텐츠를 위한 것이라면
  ...
  else { // 반면에 만일 이 요청이 동적 컨텐츠를 위한 것이라면
...
void serve_dynamic(int fd, char *filename, char *cgiargs) {
  ...
  if (Fork() == 0) { // 이 응답의 첫 번째 부분을 보낸 후에, 새로운 자식 프로세스를 fork한다.
